...
public:
  ...
  : Node("homing_mode_publisher")
  {
    ...
    motor_command_publisher_ = this->create_publisher<MotorCommand>("motor_command", QoS_RKL10V);
    RCLCPP_WARN(this->get_logger(), "motor_command publisher is created");
  ...
  { 
    ...
    while (true)
    {
      ...
      {
        if (loadcell_data_.stress[i] <= HOMING_THRESHOLD) {
